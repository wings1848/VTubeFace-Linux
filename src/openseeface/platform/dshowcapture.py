    ...
    def __init__(self):
        ...
        if lib is None or bm_lib is None:
            if platform.architecture()[0] == '32bit':
                dll_path = resolve(os.path.join("../../../bindings/dll/dshowcapture", "dshowcapture_x86.dll"))
                lib = cdll.LoadLibrary(dll_path)
                if bm_enabled:
                    dll_path = resolve(os.path.join("../../../bindings/dll/dshowcapture", "libminibmcapture32.dll"))
                    bm_lib = cdll.LoadLibrary(dll_path)
            else:
                dll_path = resolve(os.path.join("../../../bindings/dll/dshowcapture", "dshowcapture_x64.dll"))
                lib = cdll.LoadLibrary(dll_path)
                if bm_enabled:
                    dll_path = resolve(os.path.join("../../../bindings/dll/dshowcapture", "libminibmcapture64.dll"))
                    bm_lib = cdll.LoadLibrary(dll_path)
