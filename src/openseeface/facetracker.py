    ...
    while repeat or input_reader.is_open():
        ...
        try:
            ...
            for face_num, f in enumerate(faces):
                ...
                # 批量 UDP 打包：置信度、地标坐标、3D 点、特征
                # 原为逐点 struct.pack 循环，改为 numpy tobytes() 批量序列化
                packet.extend(f.lms[:, 2].tobytes())                     # 68 个置信度
                packet.extend(f.lms[:, [1, 0]].ravel().tobytes())        # 68×(y,x) 坐标
                pts3d = f.pts_3d * np.array([1, -1, -1], dtype=np.float32)  # x, -y, -z
                packet.extend(pts3d.tobytes())                           # 70×(x,-y,-z)
                if f.current_features is None:
                    f.current_features = {}
                feat_arr = np.array([f.current_features.get(feat, 0) for feat in features], dtype=np.float32)
                packet.extend(feat_arr.tobytes())                        # 14 个特征值
