"""
OpenSeeFace Face Tracker
=======================
Core face tracking module. Contains argument parsing, camera input,
ONNX model inference, and UDP output to VTube Studio.
"""
...
# ---------------------------------------------------------------------------
# Output logging helper (module-level: no args dependency)
# ---------------------------------------------------------------------------

class OutputLog:
    """Duplicates writes to a file handle and the original stream."""
...
# ---------------------------------------------------------------------------
# Argument parser factory (module-level: can be called without side effects)
# ---------------------------------------------------------------------------

def _create_parser() -> argparse.ArgumentParser:
    """Create the argument parser for the face tracker (does NOT parse)."""
    ...
    parser.add_argument("--max-feature-updates", type=int, help="This is the number of seconds after which feature min/max/medium values will no longer be updated.", default=900)
...
def main():
    """Run the face tracking loop with CLI arguments."""
            ...
            for cam in info:
                if args.list_dcaps == -1:
                    ...
                    if cam['type'] == "Blackmagic":
                        type_str = "Blackmagic: "
                    print(f"{cam['index']}: {type_str}{cam['name']}")
                if args.list_dcaps != -1 and args.list_dcaps != cam['index']:
                    continue
                for caps in cam['caps']:
                    fmt = caps['format']
                    if caps['format'] in formats:
                        fmt = formats[caps['format']]
                    if caps['minCX'] == caps['maxCX'] and caps['minCY'] == caps['maxCY']:
                        print(f"    {caps['id']}: Resolution: {caps['minCX']}x{caps['minCY']} FPS: {unit / caps['maxInterval']:.3f}-{unit / caps['minInterval']:.3f} Format: {fmt}")
                    else:
                        print(f"    {caps['id']}: Resolution: {caps['minCX']}x{caps['minCY']}-{caps['maxCX']}x{caps['maxCY']} FPS: {unit / caps['maxInterval']:.3f}-{unit / caps['minInterval']:.3f} Format: {fmt}")
        else:
    ...
    if args.benchmark > 0:
        ...
        im = cv2.imread(os.path.join(model_base_path, "benchmark.png"), cv2.IMREAD_COLOR)
        for model_type in [3, 2, 1, 0, -1, -2, -3]:
            tracker = Tracker(224, 224, threshold=0.1, max_threads=args.max_threads, silent=True, model_type=model_type, model_dir=args.model_dir, no_gaze=(model_type == -1), detection_threshold=0.1, max_feature_updates=900, static_model=True if args.no_3d_adapt == 1 else False)
            tracker.detected = 1
    ...
    try:
        ...
        while repeat or input_reader.is_open():
            ...
            if first:
                ...
                tracker = Tracker(width, height, threshold=args.threshold, max_threads=args.max_threads, silent=False if args.silent == 0 else True, model_type=args.model, model_dir=args.model_dir, no_gaze=False if args.gaze_tracking != 0 and args.model != -1 else True, detection_threshold=args.detection_threshold, max_feature_updates=args.max_feature_updates, static_model=True if args.no_3d_adapt == 1 else False, try_hard=args.try_hard == 1)
            ...
            try:
                ...
                for face_num, f in enumerate(faces):
                    ...
                    # Batch UDP packing: confidence, landmarks, 3D points, features
                    packet.extend(f.lms[:, 2].tobytes())
                    packet.extend(f.lms[:, [1, 0]].ravel().tobytes())
                    pts3d = f.pts_3d * np.array([1, -1, -1], dtype=np.float32)
                    packet.extend(pts3d.tobytes())
                    if f.current_features is None:
                        f.current_features = {}
                    feat_arr = np.array([f.current_features.get(feat, 0) for feat in features], dtype=np.float32)
                    packet.extend(feat_arr.tobytes())
                    ...
                    if args.pnp_points != 0 and (args.visualize != 0 or out is not None) and f.rotation is not None:
                        ...
                        for [(px, py)] in projected[0]:
                            px = int(px + 0.5)
                            py = int(py + 0.5)
                            if not (px < 0 or py < 0 or px >= height or py >= width):
                                frame[int(px), int(py)] = (0, 255, 255)
                            px += 1
                            if not (px < 0 or py < 0 or px >= height or py >= width):
                                frame[int(px), int(py)] = (0, 255, 255)
                            py += 1
                            ...
                            if not (px < 0 or py < 0 or px >= height or py >= width):
                                frame[int(px), int(py)] = (0, 255, 255)
                    if log is not None:
                        for (lx, ly, lz) in f.pts_3d:
                            log.write(f",{lx},{-ly},{-lz}")
                    ...
                    if cv2.waitKey(1) & 0xFF == ord('q'):
                        if args.dump_points != "" and faces is not None and len(faces) > 0:
                            ...
                            pairs = [
                                ...
                                (21, 22), (31, 35), (32, 34), (36, 45), (37, 44), (38, 43),
                                (39, 42), (40, 47), (41, 46), (48, 52), (49, 51), (56, 54),
                                (57, 53), (58, 62), (59, 61), (65, 63)
                            ]
...
# ---------------------------------------------------------------------------
# Entry point for `python -m openseeface` (non-CLI path) and direct execution
# ---------------------------------------------------------------------------
